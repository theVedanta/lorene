...
router.get("/", async (req, res) => {
  const invoices = await Quotation.find({ user: req.user.id }).sort({
    _id: -1,
  });
  const user = await User.findById(req.user.id);
  res.render("quotations/display", {
    invoices: invoices,
    user: user,
...
router.post("/add", checkVerified, async (req, res) => {
  ...
  const invNr = `quo_${invoices.length + 1}`;
  ...
  try {
    await Quotation.create(invoice);
...
router.get("/view/:id", checkVerified, async (req, res) => {
  const invoice = await Quotation.findOne({ invoiceID: req.params.id });
...
router.get("/edit/:id", checkVerified, async (req, res) => {
  ...
  try {
    return res.render("quotations/edit", {
      user: user,
      invoice: invoice,
      custs: custs,
      taxes: taxes,
...
router.post("/download/:id", async (req, res) => {
  ...
  res.redirect(`/quotations/save/${invoice.invoiceID}`);
});
...
async function checkVerified(req, res, next) {
  ...
  if (user.verified) {
    ...
    res.redirect("/quotations?verErr=Verify Account to use services");
  }
}
